...
| `CHANGELOG.md` (top-level, repo root) | The user-facing aggregated changelog. Regenerated by release-please on every release (managed by the `app` package's `changelog-path: "/CHANGELOG.md"` config — the `app` strategy aggregates all PRs and writes here). Do NOT edit by hand — release-please will overwrite it on the next release PR. |
| `crates/*/CHANGELOG.md` (per-crate) | The crate-consumer-facing changelogs. Regenerated by release-please per crate, listing only the PRs that touched that crate. Useful for downstream users of `bk-core` etc. who depend on these crates from crates.io. |
...
### The dual-changelog design (top-level + per-crate)

Talon maintains **two kinds of CHANGELOG** for two different audiences:

1. **`/CHANGELOG.md` at the repo root** — for **end users of the Tauri app**. Lists every user-facing PR (frontend, Rust, CI, anything that ships in the binary) aggregated across all crates. This is the changelog the GitHub release page links to and that `docs/release-process.md` §2.1 instructs reviewers to sanity-check.

2. **`crates/<name>/CHANGELOG.md` for each of the 8 crates** — for **downstream Rust users** who depend on `bk-core` etc. directly from crates.io. Lists only the PRs that touched that specific crate. These are independent of each other; a change in `bk-events` doesn't add an entry to `bk-core`'s changelog.

**How release-please produces this:**

- The top-level `/CHANGELOG.md` is produced by the **`app` package** in `release-please-config.json`. The `app` package's strategy doesn't filter by path (it's a "Rust" strategy that aggregates all commits since the last `app` version bump), and its `changelog-path: "/CHANGELOG.md"` config tells release-please to write the aggregated changelog to the repo root instead of `app/CHANGELOG.md`. The leading `/` is significant: it tells `addPath()` to treat the path as repo-root-relative rather than package-relative.

- The 7 crate-level changelogs are produced by the 7 crate packages. Each crate's default `changelog-path: "CHANGELOG.md"` writes to `crates/<name>/CHANGELOG.md`. The strategy filters by path — only commits that touched that crate's files get included.

**Why not just one CHANGELOG.md?** Because the audiences are different. A user downloading `Talon_0.3.0_amd64.deb` cares about every visible change in the app. A Rust developer doing `cargo add bk-core` in their own project cares only about changes to `bk-core`'s public API. Conflating them would either clutter the top-level with crate-internal changes, or omit relevant crate-internal changes from downstream visibility.

**Trade-off:** 2 × 8 = 9 changelog files maintained in lockstep, but release-please does the syncing automatically — there is no per-release manual editing.

**Historical note (2026-07-23):** prior to v0.2.2, the top-level `/CHANGELOG.md` did not exist. The `app` package wrote to `app/CHANGELOG.md` (the default `changelog-path` relative to `app/`). The aggregated view lived at `app/CHANGELOG.md` rather than at the repo root. PR #78 (this PR) moves it to `/CHANGELOG.md` so it's the first file a visitor to the GitHub repo sees, and deletes the now-stale `app/CHANGELOG.md` (the new `/CHANGELOG.md` was bootstrapped with a copy of its full content).
...
#    Also add an entry to the top-level /CHANGELOG.md (the aggregated
#    user-facing view, managed by the `app` package's `changelog-path`
#    config; see §3.x). The top-level entry should aggregate the same
#    PRs the per-crate entries list.
