...
| File | Purpose |
|---|---|
| `release-please-config.json` | Declares 8 packages (one per Cargo crate) with their `component` names + 2 plugins: `cargo-workspace` (builds the crate dependency graph, `merge: false` because `linked-versions` does the merge) and `linked-versions` (groupName `talon`, components = all 8 crate names) which keeps all 8 crates on a single version line. The `extra-files` rule stamps the unified version into `app/tauri.conf.json` and `ui/package.json` after the bump. |
| `.release-please-manifest.json` | One entry per crate, all starting at `0.1.0`. Updated on every release PR. |
| `Cargo.toml` `[workspace.package] version` | Single source of truth for the semver math. release-please's `cargo-workspace` plugin reads conventional commits, computes the next version (one set of math, applied to all 8 crates via `linked-versions`), and writes it to each crate's `Cargo.toml` `[package] version`. |
